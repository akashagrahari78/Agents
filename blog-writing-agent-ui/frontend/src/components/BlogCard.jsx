...
export default function BlogCard({ blog, onDelete }) {
  ...
  const modeStyle = modeColors[blog.settings?.mode] || modeColors.hybrid
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
        <span style={{ fontSize: '0.7rem', fontWeight: 600, padding: '0.25rem 0.6rem', borderRadius: '99px', textTransform: 'uppercase', backgroundColor: modeStyle.bg, color: modeStyle.color, border: `1px solid ${modeStyle.border}` }}>
          {blog.settings?.mode?.replace('_', ' ') || 'Hybrid'}
        </span>
        <button
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); onDelete(blog.id); }}
      ...
      <Link to={`/blog/${blog.id}`} style={{ flex: 1 }}>
